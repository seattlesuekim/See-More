class Post < ActiveRecord::Base
  ...
  def self.create_tumblr_post(post)
    ...
    case post['type']
      when 'video'
        author.posts << self.create(body: post["player"].first["embed_code"],  title: post["source_title"], posted_at: post["date"])
      when 'text'
        author.posts << self.create(body: post["player"], title: post["title"], posted_at: post["date"])
    end
  end
